...
describe('ApiService', () => {
  ...
  it('prefixes GET with /api/v1/', () => {
    ...
    const req = http.expectOne('http://localhost:5000/api/v1/projects');
  ...
  it('prefixes PATCH and DELETE with /api/v1/', () => {
    ...
    expect(patch.request.url).toBe('http://localhost:5000/api/v1/projects/1');
    patch.flush({});
    const del = http.expectOne((r) => r.method === 'DELETE');
    expect(del.request.url).toBe('http://localhost:5000/api/v1/projects/1');
    del.flush({});
  });
});
